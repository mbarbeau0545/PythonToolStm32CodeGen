...
class AppAct_CodeGen():
        ...
        for idx, act_cfg in enumerate(actuators_cfg_a):
            ...
            if(str(act_cfg[0]) != EMPTY_CELL):
                ...
                if not os.path.isfile(f"{ACT_SPEC_FOLDER_FULLPATH}/{VAR_APPACT_SPEC}_{act_cfg[0]}.h"):
                    print(f"[INFO] : APACT_Codege -> Couldn't find reference for {act_cfg[0]}, Create Header/Source file")
                    cls.make_header_src_file(str(act_cfg[0]))
                else:
                    print(f"[INFO] : APACT_Codege -> Header/Source file for {act_cfg[0]} already existing, no operation")
    ...
    def make_header_src_file(cls, f_act_name:str):
        ...
        print(f"[INFO] : APACT_Codege -> Succesfully create Header and Source file for {f_act_name}")
